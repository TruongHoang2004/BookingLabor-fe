...
export default function TaskerApplyTask() {
    ...
    const handleConfirm = async () => {
        try {
            console.log(taskID);
            await taskService.applyTask(parseInt(taskID, 10)); // Gọi hàm service với task.id
                                ...
                                >
                                    ...
                                    <CardBody className="overflow-hidden px-6 py-4">
                                        <div className="space-y-4">
                                            <p className="truncate text-gray-600 text-lg flex items-center"><BiSolidCheckCircle className="text-emerald-500" /><span className="text-emerald-700 font-semibold mr-2">Skill:</span>{task.skill?.name}</p>
