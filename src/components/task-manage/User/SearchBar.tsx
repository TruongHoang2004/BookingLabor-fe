...
export default function UserSearchBar({ onSearch }: {
    ...
    const isValidFormat = (value: string) => {
        const pattern = /^[A-Z]\d{4}$/;  // Changed to match user ID format (1 letter + 4 numbers)
    ...
    const handleSearch = () => {
        ...
        if (!isValidFormat(searchValue)) {
            setError('Invalid format. User ID should be 1 letter followed by 4 numbers (e.g., A0001)');
            return;
        }
        ...
        <div className="w-full">
            <div className="relative flex items-center">
                <input
                    ...
                    placeholder="Enter User ID (e.g., A0001)"
